...
- The site root is the only page that ships JavaScript: a synchronous inline script, minified at build time from the type-checked source `src/i18n/negotiate-language.ts`, sends a browser announcing English or Arabic to `/en/` or `/ar/` before the first render. The `lang` URL marker that opts out of it is named once in `src/i18n/negotiation.ts` and shared by the script, the language selector and the 404. Every other page — the localized pages, the print routes, the Open Graph cards and the 404 — ships no script and no redirect. See `docs/adr/0008-negotiate-language-from-root.md`.
