...
- Un accent employé comme texte utilise `accent-text` ou `accent-text-strong`, tandis qu’un contenu posé sur `bg-accent` utilise `accent-foreground`. Les tokens `accent` et `accent-strong` restent réservés aux fonds et aux éléments décoratifs.
