...
Le débordement est couvert sans ajouter de missions aux données du CV : la route reconnaît uniquement le paramètre `?test-volume=pagination` et duplique alors, dans le DOM imprimable, les groupes et missions existants avec des marqueurs de test. Le mode `--pagination-test` du générateur ouvre cette URL et écrit un artefact séparé, `dist/cv/CV.pagination-test.pdf`, préparé par `npm test`. Le build nominal n’emploie ni ce paramètre ni ce mode et continue de produire uniquement `dist/cv/CV.pdf` depuis les données réelles.
