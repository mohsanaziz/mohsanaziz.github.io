...
export const fonts = [
  {
    label: 'Noto Sans Arabic latin subset',
  ...
  {
    label: 'Noto Sans Arabic arabic subset',
...
function fontPath(directory, fileName) {
  return directory instanceof URL ? new URL(fileName, directory) : join(directory, fileName);
}

export async function provisionFont(font, { directory = fontDirectory, fetchFont = fetch } = {}) {
  const destination = fontPath(directory, font.fileName);

  try {
    const currentFont = await readFile(destination);

    if (sha256(currentFont) === font.expectedSha256) {
      console.log(`${font.label} is ready.`);
      return;
    }
  } catch (error) {
  ...
  if (!response.ok) {
    throw new Error(`Unable to download ${font.label}: ${response.status} ${response.statusText}`);
  }
  ...
  if (downloadedSha256 !== font.expectedSha256) {
    throw new Error(`Unexpected ${font.label} SHA-256: ${downloadedSha256} (expected ${font.expectedSha256})`);
  }

  await mkdir(directory, { recursive: true });
  await writeFile(destination, downloadedFont);
  console.log(`Downloaded and verified ${font.label}.`);
}

export async function provisionFonts() {
  await rm(fontPath(fontDirectory, 'Satoshi.woff2'), { force: true });
